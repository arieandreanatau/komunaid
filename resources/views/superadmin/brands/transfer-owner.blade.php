...
@php $pageTitle = 'Transfer Ownership - ' . $brand->name @endphp

@section('content')
<div class="space-y-6 max-w-3xl">
    <div>
        <a href="{{ route('superadmin.brands.show', $brand) }}" class="text-sm text-[#126BFF] hover:text-[#0B2D89] transition inline-flex items-center gap-1">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            Kembali ke Detail Brand
        </a>
        <h1 class="text-2xl font-bold text-[#0B2D89] mt-2">Transfer Ownership</h1>
        <p class="text-[#64748B] text-sm mt-1">{{ $brand->name }}</p>
    </div>
    ...
    <div class="bg-[#F59E0B]/10 border border-[#F59E0B] rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
            ...
            <p class="text-sm text-[#64748B] mt-1">Anda akan mentransfer kepemilikan brand ini dari <strong class="text-[#0F172A]">{{ $brand->owner->name }}</strong> ke user lain. Tindakan ini tidak dapat dibatalkan.</p>
        </div>
    </div>

    <div class="bg-white rounded-xl shadow-sm p-6">
        <form method="POST" action="{{ route('superadmin.brands.transfer-owner', $brand) }}">
            @csrf
            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-[#0F172A] mb-1">Owner Saat Ini</label>
                    <div class="px-4 py-2.5 bg-komuna-surface rounded-xl border border-[#E2E8F0] text-sm text-[#64748B]">{{ $brand->owner->name }}</div>
                </div>
                <div>
                ...
                <button type="submit" class="w-full bg-[#126BFF] text-white px-4 py-2.5 rounded-xl font-semibold hover:bg-[#0B2D89] transition text-sm shadow-sm" onclick="return confirm('Transfer kepemilikan brand ini?')">Transfer Kepemilikan</button>
            </div>
        </form>
    </div>
</div>
