@php $pageTitle = 'Detail Community Owner' @endphp

@extends('layouts.admin')

@section('content')
<div class="mb-6">
    <a href="{{ route('superadmin.community-owners.index') }}" class="inline-flex items-center gap-1.5 text-sm text-[#126BFF] hover:text-[#0B2D89] font-medium transition">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
        Kembali ke Community Owners
    </a>
</div>
@if($user->banned_at)
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="bg-white rounded-xl shadow-sm border border-[#E2E8F0] p-6">
        <div class="flex items-center gap-4 mb-6">
            <div class="w-14 h-14 rounded-full bg-[#0B2D89] text-white flex items-center justify-center text-xl font-bold">
        ...
        <div class="grid grid-cols-3 gap-3 mb-6">
            <div class="bg-[#EEF7FF] rounded-xl p-4 text-center">
                <div class="text-2xl font-bold text-[#0B2D89]">{{ $communitiesCount }}</div>
                <div class="text-xs text-[#64748B] mt-1">Communities</div>
            </div>
            <div class="bg-[#EEF7FF] rounded-xl p-4 text-center">
                <div class="text-2xl font-bold text-[#126BFF]">{{ $totalMembers }}</div>
                <div class="text-xs text-[#64748B] mt-1">Members</div>
            </div>
            <div class="bg-[#EEF7FF] rounded-xl p-4 text-center">
                <div class="text-2xl font-bold text-[#25B9F2]">{{ $totalEvents }}</div>
                <div class="text-xs text-[#64748B] mt-1">Events</div>
            </div>
        </div>
        <div class="space-y-2">
            @if($communitiesCount > 0)
                <a href="{{ route('superadmin.community-owners.communities', $user) }}" class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-[#126BFF] text-white text-sm font-medium rounded-lg hover:bg-[#0B2D89] transition">View Communities</a>
            @endif
            @if($user->banned_at)
                <form method="POST" action="{{ route('superadmin.community-owners.activate', $user) }}">
                    @csrf
            ...
            @else
                <form method="POST" action="{{ route('superadmin.community-owners.suspend', $user) }}">
                    @csrf
                ...
                <form method="POST" action="{{ route('superadmin.community-owners.ban', $user) }}">
                    @csrf
    ...
    <div class="lg:col-span-2 space-y-6">
        <div class="bg-white rounded-xl shadow-sm border border-[#E2E8F0] p-6">
            <h3 class="text-lg font-bold text-[#0F172A] mb-4">Communities</h3>
            @if($user->ownedCommunities->isEmpty())
                <p class="text-sm text-[#64748B] text-center py-6">Belum ada komunitas.</p>
            @else
                <div class="space-y-2">
                    @foreach($user->ownedCommunities as $community)
                        <div class="flex items-center justify-between p-3 bg-[#EEF7FF]/50 rounded-lg border border-[#E2E8F0]">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-lg bg-[#0B2D89] text-white flex items-center justify-center text-xs font-bold">{{ strtoupper(substr($community->name, 0, 1)) }}</div>
                                <div>
                                    <p class="text-sm font-medium text-[#0F172A]">{{ $community->name }}</p>
                                    <p class="text-xs text-[#64748B]">{{ $community->category->name ?? '-' }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                @include('superadmin.partials.status-badge', ['status' => $community->status])
                                <a href="{{ route('superadmin.communities.show', $community) }}" class="text-xs text-[#126BFF] hover:text-[#0B2D89] font-medium">Detail</a>
                            </div>
                        </div>
                    @endforeach
                </div>
