@php $pageTitle = 'Brand Owners' @endphp
...
<div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h1 class="text-2xl font-bold text-[#0F172A]">Brand Owners</h1>
        <p class="text-[#64748B] mt-1">Kelola semua pemilik brand di platform</p>
    </div>
    <a href="{{ route('superadmin.brand-owners.export', request()->query()) }}"
...
<div class="bg-white rounded-xl shadow-sm border border-[#E2E8F0] p-4 mb-6">
    <form method="GET" action="{{ route('superadmin.brand-owners.index') }}" class="flex flex-wrap gap-3 items-end">
        ...
        @if(request('search'))
            <a href="{{ route('superadmin.brand-owners.index') }}" class="px-4 py-2 bg-komuna-border-soft text-[#64748B] text-sm font-medium rounded-lg hover:bg-komuna-border transition">Reset</a>
        @endif
    </form>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-[#E2E8F0]">
        ...
        <table class="min-w-full divide-y divide-[#E2E8F0]">
            <thead class="bg-[#EEF7FF]">
                <tr>
                    ...
                    <th class="px-5 py-3.5 text-left text-xs font-semibold text-[#0B2D89] uppercase tracking-wider">Brands</th>
                    <th class="px-5 py-3.5 text-left text-xs font-semibold text-[#0B2D89] uppercase tracking-wider">Actions</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-[#E2E8F0]">
                @forelse($brandOwners as $owner)
                    <tr class="hover:bg-[#EEF7FF]/50 transition">
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 rounded-full bg-[#126BFF] text-white flex items-center justify-center text-sm font-bold flex-shrink-0">
                                    {{ strtoupper(substr($owner->name, 0, 1)) }}
                                </div>
                                <a href="{{ route('superadmin.brand-owners.show', $owner) }}" class="text-sm font-semibold text-[#0F172A] hover:text-[#126BFF] transition">{{ $owner->name }}</a>
                            </div>
                        </td>
                        ...
                        <td class="px-5 py-4">
                            <span class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-[#126BFF]/10 text-[#126BFF] text-sm font-bold">{{ $owner->ownedBrands->count() }}</span>
                        </td>
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-1.5">
                                <a href="{{ route('superadmin.brand-owners.show', $owner) }}" class="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-[#126BFF] bg-[#126BFF]/5 rounded-lg hover:bg-[#126BFF]/10 transition">View</a>
                                @if($owner->banned_at)
                                    <form method="POST" action="{{ route('superadmin.brand-owners.activate', $owner) }}">
                                        @csrf
                                ...
                                @else
                                    <form method="POST" action="{{ route('superadmin.brand-owners.suspend', $owner) }}">
                                        @csrf
                                    ...
                                    <form method="POST" action="{{ route('superadmin.brand-owners.ban', $owner) }}">
                                        @csrf
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-5 py-16 text-center">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-[#0F172A]">Tidak ada brand owner ditemukan</h3>
    ...
    @if($brandOwners->hasPages())
        <div class="px-5 py-4 border-t border-[#E2E8F0]">{{ $brandOwners->withQueryString()->links() }}</div>
    @endif
</div>
@endsection
