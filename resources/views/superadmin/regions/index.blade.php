...
<div class="flex justify-between items-center mb-8">
    <div>
        <h1 class="text-2xl font-bold text-gray-900">Master Regions</h1>
        <p class="text-gray-600">Kelola data region/wilayah</p>
    </div>
...
<div class="bg-white rounded-xl shadow-sm p-6 mb-6">
    <form method="GET" action="{{ route('superadmin.regions.index') }}" class="flex flex-wrap gap-3 items-end">
        ...
        <div class="min-w-[150px]">
            ...
            <select name="province" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-emerald-500 focus:border-emerald-500">
        ...
        <a href="{{ route('superadmin.regions.index') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg text-sm hover:bg-gray-300">Reset</a>
    </form>
</div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($regions as $region)
                    <tr>
                        <td class="px-4 py-3 text-sm font-medium text-gray-900">{{ $region->name }}</td>
                        <td class="px-4 py-3 text-sm text-gray-600">{{ $region->code ?? '-' }}</td>
                        <td class="px-4 py-3 text-sm text-gray-600">{{ $region->province ?? '-' }}</td>
                        <td class="px-4 py-3">
                            @if($region->is_active)
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-8 text-center text-gray-500">Belum ada region.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
