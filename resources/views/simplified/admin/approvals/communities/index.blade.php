...
<h1 class="text-2xl font-bold text-gray-900 mt-2 mb-4">Approval Komunitas</h1>

@php
    $statuses = ['pending_approval','need_revision','approved','rejected','suspended'];
@endphp
<div class="flex flex-wrap gap-2 mb-4 text-sm">
    @foreach($statuses as $s)
        <a href="{{ route('simplified.admin.approvals.communities.index', ['status' => $s]) }}" class="px-3 py-1.5 rounded-lg {{ $status===$s ? 'bg-indigo-600 text-white' : 'bg-white border border-gray-200' }}">{{ str_replace('_',' ',$s) }}</a>
    @endforeach
</div>
            ...
            <tbody>
                @foreach($items as $c)
                    <tr class="border-t">
                        ...
                        <td class="px-4 py-2"><a href="{{ route('simplified.admin.approvals.communities.show', $c->id) }}" class="text-indigo-600">Detail</a></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
