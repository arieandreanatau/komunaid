...
@section('title', 'KomunaID — Connect, Community, Grow')
        ...
        <div class="text-center">
            @if($sections->has('hero'))
                <h1 class="text-4xl sm:text-5xl lg:text-6xl font-extrabold mb-6 tracking-tight">
                    {!! $sections['hero']->title ?? 'Temukan, Bangun, dan Kembangkan Komunitasmu' !!}
                </h1>
                <p class="text-xl sm:text-2xl text-blue-200 mb-4 font-light">
                    {!! $sections['hero']->subtitle ?? 'KomunaID menghubungkan member, komunitas, brand, dan perusahaan dalam satu ekosistem kolaborasi.' !!}
                </p>
            @else
                <h1 class="text-4xl sm:text-5xl lg:text-6xl font-extrabold mb-6 tracking-tight">
            ...
            <div class="flex flex-col sm:flex-row justify-center gap-4 mt-8">
                ...
                @endguest
                <a href="{{ route('communities.directory') }}" class="border-2 border-white text-white px-8 py-3 rounded-xl text-lg font-semibold hover:bg-white/10 transition">
                    Jelajahi Komunitas
                </a>
                <a href="{{ route('about') }}" class="border-2 border-white/50 text-white/80 px-8 py-3 rounded-xl text-lg font-semibold hover:bg-white/5 transition">
                    Buat Komunitas
                </a>
            </div>
        </div>
...
@if(($recommendedCommunities ?? collect())->isNotEmpty())
<section class="bg-komuna-light py-16">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @each('public.partials.community-card', $recommendedCommunities, 'community')
        </div>
        <div class="text-center mt-8">
            <a href="{{ route('communities.directory') }}" class="inline-block bg-komuna-blue text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-komuna-navy transition">
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @each('public.partials.event-card', $upcomingEvents, 'event')
    </div>
    <div class="text-center mt-8">
        <a href="{{ route('events.index') }}" class="inline-block bg-komuna-blue text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-komuna-navy transition">
            Lihat Semua Event
        </a>
    </div>
</section>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
            ...
            <div class="text-white">
                ...
                <a href="{{ route('about') }}" class="inline-block border-2 border-white text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-white/10 transition">Ajukan Kolaborasi</a>
            </div>
        </div>
    </div>
</section>
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @each('public.partials.blog-card', $latestBlogs, 'blog')
    </div>
    <div class="text-center mt-8">
        <a href="{{ route('blogs.index') }}" class="inline-block bg-komuna-blue text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-komuna-navy transition">
            Lihat Semua Blog
        </a>
    </div>
</section>
