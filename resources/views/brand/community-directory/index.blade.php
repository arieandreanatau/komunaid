...
<div class="mb-8">
    ...
    <p class="text-komuna-muted">Lihat komunitas approved yang tersedia untuk kolaborasi.</p>
</div>

@if($communities->count() > 0)
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($communities as $community)
            <a href="{{ route('brand.community-directory.show', $community) }}" class="block bg-white rounded-2xl shadow-sm border border-komuna-border-soft p-5 hover:shadow-md transition">
                ...
                @if($community->description)
                    <p class="text-sm text-komuna-muted mb-3 line-clamp-2">{{ Str::limit($community->description, 100) }}</p>
                @endif
                ...
                <div class="mt-3 flex items-center gap-2">
                    <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-komuna-success-soft text-komuna-success">Approved</span>
...
@else
    <div class="bg-white rounded-2xl shadow-sm border border-komuna-border-soft p-8 text-center">
        ...
        <p class="text-komuna-muted text-sm">Belum ada komunitas yang approved.</p>
    </div>
@endif
@endsection
