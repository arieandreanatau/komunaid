...
<div class="mb-8">
    ...
    <p class="text-komuna-muted">Kirim proposal kolaborasi ke komunitas.</p>
</div>
...
@if($brands->isEmpty())
    <div class="bg-white rounded-2xl shadow-sm border border-komuna-border-soft p-8 text-center">
        ...
        <p class="text-komuna-muted text-sm mb-4">Buat brand terlebih dahulu sebelum mengajukan proposal.</p>
        <a href="{{ route('brand.brands.create') }}" class="inline-block bg-indigo-600 text-white px-5 py-2 rounded-lg text-sm font-medium hover:bg-indigo-700 transition">Buat Brand</a>
    </div>
@else
    <form method="POST" action="{{ route('brand.proposals.store') }}" enctype="multipart/form-data" class="max-w-3xl">
        @csrf
        <div class="bg-white rounded-2xl shadow-sm border border-komuna-border-soft p-6 space-y-6">
            <div>
                <label class="block text-sm font-medium text-komuna-text mb-1">Brand Pengaju <span class="text-komuna-danger">*</span></label>
                <select name="proposer_id" class="w-full border border-komuna-border rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-komuna-blue" required>
                    <option value="">Pilih Brand</option>
                    @foreach($brands as $brand)
                        <option value="{{ $brand->id }}" data-type="brand" {{ old('proposer_id') == $brand->id ? 'selected' : '' }}>{{ $brand->name }}</option>
                    @endforeach
                </select>
                <input type="hidden" name="proposer_type" value="brand">
            ...
            <div>
                ...
                @error('collaboration_type_id') <p class="text-komuna-danger text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            ...
            <div>
                ...
                @error('attachment') <p class="text-komuna-danger text-xs mt-1">{{ $message }}</p> @enderror
            </div>
        </div>
        ...
        <div class="flex items-center gap-3 mt-6">
            ...
            <a href="{{ route('brand.proposals.index') }}" class="text-komuna-muted hover:text-komuna-text text-sm font-medium">Batal</a>
        </div>
    </form>
@endif
@endsection
