...
@php $pageTitle = 'Edit Galeri' @endphp
...
<div class="max-w-2xl mx-auto">
    <div class="mb-8">
        ...
        <h1 class="text-2xl sm:text-3xl font-bold text-komuna-navy">Edit Galeri</h1>
        <p class="text-komuna-muted">Perbarui informasi foto kegiatan.</p>
    </div>

    <div class="bg-komuna-light rounded-2xl border border-komuna-border p-6">
        <form method="POST" action="{{ route('member.galleries.update', $gallery) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="space-y-5">
                <div>
                    <label class="block text-sm font-medium text-komuna-text mb-1">Foto Saat Ini</label>
                    <div class="mt-1 mb-3">
                        <img src="{{ asset('storage/' . $gallery->image) }}" alt="{{ $gallery->caption }}" class="w-full max-h-64 object-cover rounded-lg border border-komuna-border" id="current-image">
                    </div>
                    <label for="image" class="block text-sm font-medium text-komuna-text mb-1">Ganti Foto</label>
                ...
                <div>
                    ...
                    <textarea name="caption" id="caption" rows="3" placeholder="Tulis keterangan foto..."
                              class="block w-full border border-komuna-border rounded-lg px-3 py-2 text-sm text-komuna-text bg-komuna-surface focus:ring-2 focus:ring-komuna-cyan focus:border-komuna-cyan">{{ old('caption', $gallery->caption) }}</textarea>
                    @error('caption')
                        <p class="mt-1 text-sm text-komuna-danger">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div>
                        ...
                        <select name="community_id" id="community_id"
                            ...
                            @foreach($communities as $community)
                                <option value="{{ $community->id }}" {{ old('community_id', $gallery->community_id) == $community->id ? 'selected' : '' }}>{{ $community->name }}</option>
                            @endforeach
                        </select>
                        @error('community_id')
                            <p class="mt-1 text-sm text-komuna-danger">{{ $message }}</p>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="activity_date" id="activity_date" value="{{ old('activity_date', $gallery->activity_date?->format('Y-m-d')) }}"
                               class="block w-full border border-komuna-border rounded-lg px-3 py-2 text-sm text-komuna-text bg-komuna-surface focus:ring-2 focus:ring-komuna-cyan focus:border-komuna-cyan">
                        @error('activity_date')
                            <p class="mt-1 text-sm text-komuna-danger">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="visibility" id="visibility" required
                                class="block w-full border border-komuna-border rounded-lg px-3 py-2 text-sm text-komuna-text bg-komuna-surface focus:ring-2 focus:ring-komuna-cyan focus:border-komuna-cyan">
                            <option value="public" {{ old('visibility', $gallery->visibility) === 'public' ? 'selected' : '' }}>Publik</option>
                            <option value="friends" {{ old('visibility', $gallery->visibility) === 'friends' ? 'selected' : '' }}>Teman</option>
                            <option value="private" {{ old('visibility', $gallery->visibility) === 'private' ? 'selected' : '' }}>Privat</option>
                        </select>
                        @error('visibility')
                            <p class="mt-1 text-sm text-komuna-danger">{{ $message }}</p>
                        @enderror
