# SDLC Documentation - KomunaID

## Document Index

| File | Deskripsi |
|------|-----------|
| [00-project-overview.md](00-project-overview.md) | Ringkasan project, visi, misi, tech stack |
| [01-requirements.md](01-requirements.md) | Functional & Non-functional requirements, User stories |
| [02-database-design.md](02-database-design.md) | Schema database, relationships |
| [03-modules.md](03-modules.md) | Module breakdown, file structure, development phases |
| [04-architecture.md](04-architecture.md) | System architecture, routing, auth flow |
| [05-development-phases.md](05-development-phases.md) | Development phases timeline |
| [06-roles-permissions.md](06-roles-permissions.md) | Roles, permissions, Spatie setup |
| [07-api-endpoints.md](07-api-endpoints.md) | All route endpoints |
| [08-ui-ux-design.md](08-ui-ux-design.md) | Wireframe, color system, layout patterns |
| [09-module-checklist.md](09-module-checklist.md) | Checklist per modul |
| [10-seed-data.md](10-seed-data.md) | Seed data plan |
| [user-guide.md](user-guide.md) | Panduan pengguna (Member, CO, BO, Superadmin) |
| [05-deployment/Runbook.md](05-deployment/Runbook.md) | Deployment runbook & production guide |
| [05-deployment/Troubleshooting.md](05-deployment/Troubleshooting.md) | Troubleshooting guide |

## Development Order

1. Phase 1: Project Setup & Foundation
2. Phase 2: Public Pages
3. Phase 3: Superadmin Module
4. Phase 4: Community Owner Module
5. Phase 5: Brand Owner Module
6. Phase 6: Member Module
7. Phase 7: Wallet System
8. Phase 8: Polish & Documentation
9. Phase 9: Deployment & Production
