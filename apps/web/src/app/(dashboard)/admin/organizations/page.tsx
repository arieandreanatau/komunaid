...
export default function AdminOrganizationsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin-organizations', search, page, filter],
    queryFn: () =>
      api
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold text-gray-900">Organization Approval Queue</h1>
      ...
      {isLoading ? (
              ...
              <div key={o.id} className="card flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {o.description?.slice(0, 100) || 'No description'}
                  </p>
                  <span
                    className={cn(
                      'mt-1 inline-block rounded-full px-2 py-0.5 text-xs font-medium',
                      o.status === 'approved' && 'bg-teal/10 text-teal',
        ...
        <EmptyState title="Tidak ada organisasi" />
      )}
    </div>
  );
}
