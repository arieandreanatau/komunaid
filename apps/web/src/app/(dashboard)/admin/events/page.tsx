...
export default function AdminEventsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      api
        .get('/admin/events', { params: { search, page, limit: 20, status: filter || undefined } })
        .then((r) => r.data.data),
  });
  ...
  const rejectMutation = useMutation({
    mutationFn: (id: string) => api.post(`/admin/events/${id}/reject`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['admin-events'] }),
  });
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold text-gray-900">Event Moderation</h1>
      ...
      {isLoading ? (
              ...
              <div key={e.id} className="card flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {e.community?.name || 'Unknown'} &middot; {formatDate(e.startDate || e.date)}
                  </p>
                  <span
                    className={cn(
                      'mt-1 inline-block rounded-full px-2 py-0.5 text-xs font-medium',
                      e.status === 'approved' && 'bg-teal/10 text-teal',
        ...
        <EmptyState title="Tidak ada event" />
      )}
    </div>
  );
}
