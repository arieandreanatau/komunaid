...
interface OrgItem {
  ...
  industry: string | null; status: string; visibility: string; adminNote: string | null;
...
export default function OrganizationsPage() {
  ...
  const [modal, setModal] = useState<{ type: "revision" | "reject"; orgId: string } | null>(null);
  ...
  const handleApprove = async (id: string) => {
    setActionLoading(id);
    try { await api.put(`/admin/organizations/${id}/approve`); fetchOrgs(); }
    finally { setActionLoading(null); }
  };
  const handleReject = async (id: string, note: string) => {
    setActionLoading(id);
    try { await api.patch(`/admin/organizations/${id}/reject`, { note }); setModal(null); fetchOrgs(); }
    finally { setActionLoading(null); }
  };
  const handleRevision = async (id: string, note: string) => {
    setActionLoading(id);
    try { await api.patch(`/admin/organizations/${id}/request-revision`, { note }); setModal(null); fetchOrgs(); }
    finally { setActionLoading(null); }
  };
  const handleSuspend = async (id: string) => {
    setActionLoading(id);
    try { await api.put(`/admin/organizations/${id}/suspend`); fetchOrgs(); }
    finally { setActionLoading(null); }
  };
  const handleRestore = async (id: string) => {
    setActionLoading(id);
    try { await api.put(`/admin/organizations/${id}/restore`); fetchOrgs(); }
    finally { setActionLoading(null); }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="relative w-full md:w-80">
          ...
          <input type="text" placeholder="Cari organisasi..." value={search}
      ...
      {loading ? (
        <div className="space-y-3">{Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="bg-white rounded-xl p-6 shadow-sm animate-pulse"><div className="h-4 bg-gray-200 rounded w-1/3 mb-2" /><div className="h-3 bg-gray-200 rounded w-1/4" /></div>
        ))}</div>
      ) : orgs.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-xl shadow-sm"><p className="text-gray-500">Tidak ada organisasi</p></div>
      ) : (
        <>
          <div className="space-y-3">
            {orgs.map((org) => (
              <div key={org.id} className="bg-white rounded-xl shadow-sm p-5">
                <div className="flex flex-col md:flex-row md:items-start gap-4">
                  <div className="flex items-start gap-4 flex-1 min-w-0">
                    {org.logo ? <img src={org.logo} alt={org.name} className="h-12 w-12 rounded-lg object-cover shrink-0" /> :
                      <div className="h-12 w-12 rounded-lg bg-komuna-teal/10 flex items-center justify-center shrink-0"><span className="text-komuna-teal font-bold text-lg">{org.name[0]}</span></div>}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-semibold text-komuna-navy">{org.name}</h3>
                        <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${statusColors[org.status]}`}>{statusLabels[org.status]}</span>
                      </div>
                      <p className="text-sm text-gray-500 mt-0.5">{org.owner.name} &middot; {org.owner.email}</p>
                      {org.adminNote && <div className="mt-2 px-3 py-2 bg-orange-50 border border-orange-200 rounded-lg text-sm text-orange-700">Catatan: {org.adminNote}</div>}
                      <p className="text-sm text-gray-600 mt-2 line-clamp-2">{org.description || "Tidak ada deskripsi"}</p>
                      <div className="flex items-center flex-wrap gap-2 mt-2">
                        {org.categories.map((c) => <span key={c.id} className="px-2 py-0.5 bg-komuna-blue/10 text-komuna-blue text-xs rounded-full">{c.name}</span>)}
                        {org.tags.map((t, i) => <span key={i} className="px-2 py-0.5 bg-gray-100 text-gray-600 text-xs rounded-full">#{t}</span>)}
                      </div>
                      <div className="flex items-center gap-3 mt-2 text-xs text-gray-400">
                        {org.industry && <span>{org.industry}</span>}
                        <span>{org.memberCount} anggota</span>
                        <span>{org.eventCount} event</span>
                        <span>Diajukan {formatDate(org.submittedAt)}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-row md:flex-col gap-2 shrink-0">
                    {(org.status === "PENDING" || org.status === "REVISION_REQUIRED") && (
                      <>
                        <button onClick={() => handleApprove(org.id)} disabled={actionLoading === org.id}
                        ...
                        <button onClick={() => setModal({ type: "revision", orgId: org.id })} disabled={actionLoading === org.id}
                        ...
                        <button onClick={() => setModal({ type: "reject", orgId: org.id })} disabled={actionLoading === org.id}
                    ...
                    {org.status === "APPROVED" && (
                      <button onClick={() => handleSuspend(org.id)} disabled={actionLoading === org.id}
                    ...
                    {org.status === "SUSPENDED" && (
                      <button onClick={() => handleRestore(org.id)} disabled={actionLoading === org.id}
      ...
      {modal && (
        <NoteModal
          title={modal.type === "revision" ? "Minta Revisi" : "Tolak Organisasi"}
          placeholder={modal.type === "revision" ? "Jelaskan apa yang perlu diperbaiki..." : "Alasan penolakan..."}
          onConfirm={(note) => modal.type === "revision" ? handleRevision(modal.orgId, note) : handleReject(modal.orgId, note)}
          onCancel={() => setModal(null)} loading={actionLoading === modal.orgId} />
      )}
    </div>
  );
}
