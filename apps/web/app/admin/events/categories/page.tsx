...
export default function EventCategoriesPage() {
  ...
  const fetchCategories = useCallback(async () => {
    setLoading(true);
    try {
      const params: Record<string, any> = { type: "EVENT", includeInactive: showInactive };
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal?.mode === "create") {
        await api.post("/admin/categories", { ...form, type: "EVENT" });
      } else if (modal?.category) {
        await api.put(`/admin/categories/${modal.category.id}`, { ...form, type: "EVENT" });
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-komuna-navy">Event Categories</h1>
          <p className="text-sm text-gray-500 mt-1">Kelola kategori event</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-xl shadow-sm"><p className="text-gray-500">Tidak ada kategori event</p></div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {categories.map((cat) => (
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-700">Event</span>
                  </div>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
            <h3 className="text-lg font-semibold text-komuna-navy mb-4">{modal.mode === "create" ? "Tambah Kategori Event" : "Edit Kategori Event"}</h3>
