...
export default function RegisterPage() {
  ...
  useEffect(() => {
    if (!registrationSuccess && !isLoading && isAuthenticated && user) {
      router.replace(redirectTo);
    }
  }, [registrationSuccess, isLoading, isAuthenticated, user, router, redirectTo]);
      ...
      <div className="max-w-md w-full space-y-8">
        {registrationSuccess ? (
          <div className="rounded-2xl border border-green-200 bg-white p-8 text-center shadow-sm" role="status" aria-live="polite">
            ...
            <h2 className="mt-5 text-2xl font-bold text-komuna-navy">Pendaftaran Berhasil</h2>
            <p className="mt-2 text-sm text-gray-600">
              Akun Anda sudah aktif. Anda akan diarahkan ke dashboard.
            </p>
            <div className="mt-6 flex items-center justify-center gap-3 text-sm font-medium text-komuna-blue">
              <div className="h-5 w-5 animate-spin rounded-full border-2 border-komuna-blue border-t-transparent" />
              Memuat dashboard...
            </div>
          </div>
        ) : (
