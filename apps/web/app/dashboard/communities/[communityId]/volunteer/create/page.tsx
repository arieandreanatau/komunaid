...
export default function CreateCommunityVolunteerProgramPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try { await api.post(`/volunteer-programs/communities/${communityId}`, { communityId, title: form.get("title"), description: form.get("description"), location: form.get("location"), capacity: Number(form.get("capacity")), startDate: new Date(String(form.get("startDate"))).toISOString(), endDate: new Date(String(form.get("endDate"))).toISOString() }); router.push(`/dashboard/communities/${communityId}/volunteer`); } catch (requestError: any) { setError(requestError.response?.data?.message || "Program volunteer gagal dibuat."); } finally { setSubmitting(false); }
  }
