        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-6 flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Cari volunteer opportunity..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-komuna-blue focus:border-transparent text-sm" />
            </div>
            <div className="flex gap-2">
              {STATUS_OPTIONS.map((opt) => (
                <button key={opt.value} onClick={() => setStatus(opt.value)} className={`px-3 py-2 text-sm font-medium rounded-lg border transition-colors ${status === opt.value ? "bg-komuna-blue text-white border-komuna-blue" : "bg-white text-gray-600 border-gray-200 hover:border-komuna-blue/50"}`}>
                  {opt.label}
                </button>
              ))}
            </div>
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {opportunities.map((opp) => (
                  <Link key={opp.id} href={`/volunteer/${opp.slug}`} className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between mb-3">
                      <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${getStatusBadge(opp.status)}`}>{opp.status}</span>
                      {opp.activityStartDate && <span className="text-xs text-gray-500">{formatDate(opp.activityStartDate)}</span>}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      {opp.positions.map((pos) => (
                        <span key={pos.id} className="px-2 py-0.5 text-xs bg-komuna-teal/10 text-komuna-teal rounded-full">{pos.name} ({pos.requiredQty})</span>
                      ))}
                    </div>
                    <div className="flex items-center justify-between text-xs text-gray-500">
                      <span>{opp.applicationCount} pendaftar</span>
                      {opp.registrationDeadline && <span>Deadline: {formatDate(opp.registrationDeadline)}</span>}
                    </div>
                  </Link>
                ))}
              </div>
