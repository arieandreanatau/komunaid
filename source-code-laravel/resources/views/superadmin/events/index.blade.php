@extends('layouts.app')
@section('title', 'Event - Superadmin')
@section('content')
<div class="max-w-7xl mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold text-navy mb-6">Data Event</h1>
        ...
        <table class="w-full">
            <thead class="bg-soft-bg">
                <tr>
                    ...
                    <th class="text-left p-3 text-sm font-medium text-navy">Komunitas</th>
            ...
            <tbody class="divide-y">
                @forelse($events as $i => $event)
                    <tr class="hover:bg-gray-50">
                        <td class="p-3 text-sm">{{ $i + $events->firstItem() }}</td>
                        <td class="p-3 text-sm font-medium">{{ $event->title }}</td>
                        <td class="p-3 text-sm text-gray-500">{{ $event->community->name }}</td>
                        <td class="p-3 text-sm">{{ $event->start_date->format('d M Y') }}</td>
